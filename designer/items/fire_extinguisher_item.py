...
class FireExtinguisherItem(QGraphicsItem):

    # Fire Suppression & Water-Based Safety Asset Digital Twin
    # milestone -- a red UPWARD TRIANGLE body (a generic extinguisher-
    # cylinder glyph, not a vendor reproduction), deliberately a
    # different SHAPE from every sibling item (circle/square/diamond
    # already taken by Smoke-Heat Detector/Sprinkler, Manual Call
    # Point, Emergency Light respectively) so it reads as its own
    ...
    # compute_availability() -- never computed here, mirroring
    # current_availability on EmergencyLightItem exactly.
    ...
    def __init__(self, x, y, model=None):
        ...
        self.default_body_brush = QBrush(QColor(198, 40, 40))
    ...
    def _triangle(self, radius):

        return QPolygonF([
            QPointF(0, -radius), QPointF(radius * 0.87, radius * 0.5), QPointF(-radius * 0.87, radius * 0.5),
        ])
    ...
    def shape(self):
        ...
        path = QPainterPath()
        path.addPolygon(self._triangle(self.BODY_RADIUS))
        path.closeSubpath()
    ...
    def paint(self, painter, option, widget=None):
        ...
        painter.drawPolygon(self._triangle(self.BODY_RADIUS))
        ...
        painter.drawPolygon(self._triangle(self.BODY_RADIUS + 4))
