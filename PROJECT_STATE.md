...
- **Current Phase**: Multi-Camera Streaming Architecture (Phase 1) Complete — verification + light hardening of the Live CCTV pipeline for N-simultaneous-camera use, following: Live CCTV Dashboard → Digital Twin Live Occupant Marker → Live CCTV Layout Fix → Camera -> Zone Assignment V1 → System Capability Inventory → Multi-Camera Streaming Architecture (Phase 1)
- **Current Version**: v0.9.6
- **Last Completed Milestone**: Multi-Camera Streaming Architecture (Phase 1) — investigated whether the production Live CCTV pipeline already supports many simultaneous cameras (it does: `RTSPFrameSource` is already one-per-camera, `LiveCameraPipeline`/`CameraManager`/`LiveCameraViewGateway` are already dict-keyed/shared-singleton, `LiveCameraGridPanel` is already a dynamic, scrolling, no-hardcoded-count grid), proved it end to end at 1/4/16/32/100 simulated cameras with zero LAN/network dependency, added the one genuine gap found (frame timestamp now visible per tile) and two new regression-guard test files. See Session Changelog for the full investigation report. Immediately preceded by SynEvac Capability & Architecture Inventory (`docs/architecture/system_capability_inventory.md`) and Camera -> Zone Assignment V1.
- **Next Planned Milestone**: TBD. Explicitly deferred by this milestone's own scope (per direct instruction, not yet started): Camera Topology, Coverage, Virtual Zones, Cross-camera identity, World-coordinate estimation, Digital Twin enhancements. Also still open: **Open Verification Queue #1** below — real-LAN/NVR confirmation of the Live CCTV 32-camera (now proven to N=100+ offline) layout fix against Camera 7, blocked on physical access.
...
- **Total Tests**: 380 passed in this milestone's own targeted batch (RTSP/camera-manager/live-camera-pipeline/live-runtime/command-center/grid-panel/tile-widget suites, 5 subtests among them for the 1/4/16/32/100-camera scale test), zero failures. The full-suite combined count from the prior merge (2026-08-06, ~5967 across 430 files pre-merge, plus 134+84 from the Live CCTV/Digital Twin arc, never run together as one number) is **still not re-verified as a single total** — carried forward as the first recommended action of a future session, unchanged from the prior entry.
- **Known Flaky Tests**:
  ...
  - `tests/test_builder_validation.py::BuilderMainWindowValidationGatingTests::test_save_succeeds_once_error_is_resolved` — same root cause: `BuilderMainWindow._save_to()` (builder/windows/builder_main_window.py:734) calls a real unmocked `QMessageBox.critical()` when validation fails, and this test's first `_save_to()` call (expected to fail validation) never patches it, unlike its sibling test directly above which does. Both are pre-existing Builder test bugs, unrelated to any of the five milestones above.
- **Known worktree-only test artifact (not a real bug)**: `tests/test_detector_identity_unification.py::ArchitectureGuardTests::test_only_one_detector_migration_module_exists` fails ONLY when the entire test suite is run from inside a nested `.claude/worktrees/...` checkout — the guard's own `rglob("detector_migration.py")` filter deliberately excludes any match whose path contains `"worktrees"` (to avoid double-counting a stray sibling checkout), but when the *whole repo* being tested is itself under a `worktrees` directory, that filter incidentally excludes the one legitimate `models/detector_migration.py` too. Should pass normally in any run from `main` going forward (not a worktree checkout for this purpose).
- **Known Issues**:
  - **Full-suite run repeatedly blocked by an unrelated Qt modal test hang**, recurring across multiple separate sessions/attempts (Live CCTV Dashboard, Live CCTV Layout Fix, and others) — a genuine block (confirmed via `Get-Process` CPU-time sampling showing ~3.75s of actual CPU over a 59-minute run), not a slow-but-working run. No test name or stack trace has yet been recovered (stdout fully buffered on every attempt). Most likely a third, not-yet-catalogued instance of the same class of bug as the two Known Flaky Tests above. Next session should re-run with `python -u` (unbuffered) and `-x -v` specifically to catch the stall location — this has been deferred across multiple sessions now and should be prioritized.
  - **Intermittent Live Runtime startup hang, unresolved, documented not fixed.** Occurred 3 times total across real-GUI launches with the original uninstrumented launcher (40+ minutes each), CPU signature "active/spinning" not "blocked" (ruling out simple network blocking as the sole cause). Two bounded, fully-instrumented diagnostic attempts (per explicit user instruction to stop after two clean attempts) both completed cleanly with no hang and no stack trace captured. Root cause NOT identified. Instrumentation preserved at `C:\Users\riddh\.claude\jobs\99e9d6ab\tmp\diagnose_startup_hang.py` (uses `faulthandler.dump_traceback_later` armed at every startup checkpoint — `LiveRuntimeSession.start()`, `build_live_runtime()`, camera source construction, credential lookup, `RTSPFrameSource.start()`, `OpenCVFrameDecoderBackend.open()`, first `cv2.VideoCapture()` call, `runtime.start()` return, first `run_cycle()`, `on_start()` completion) for reuse if it reoccurs.

---

## Open Verification Queue

> Items here are IMPLEMENTED but NOT yet confirmed against real hardware/network. An item stays open until every one of its checkboxes is confirmed against the real system it names — implementation completeness, passing unit/regression tests, and code review do NOT close an item by themselves. Process: show this queue briefly at the start of every session before starting new work; while the LAN is disconnected, treat every open item as blocked and steer toward LAN-independent work without repeating this reminder; the first time the user reports the LAN is connected, remind them once that open items exist before starting any new LAN-dependent feature, then don't repeat the reminder for the rest of that session.

### OPEN VERIFICATION #1 — Live CCTV 32-Camera Layout Verification

**Status**: IMPLEMENTED — NOT YET VERIFIED ON REAL HARDWARE

**Implementation already completed** (see Session Changelog, "Live CCTV Layout Fix"):
- QScrollArea added to `LiveCameraGridPanel`.
- 32-camera clipping fix implemented.
- Regression tests passing (8/8 targeted, 101/101 broader Command Center suite).
- PROJECT_STATE.md updated.
- **Update (Multi-Camera Streaming Architecture, Phase 1)**: the underlying construction/ownership/grid-building architecture is now proven offline at 1/4/16/32/100 simulated cameras (`tests/test_multi_camera_streaming_e2e.py`) — this raises confidence the layout fix generalizes well past 32, but does **not** close this item; a real screenshot/observation against the actual Camera 7 stream is still required and untouched by this milestone (explicitly out of scope: "No LAN verification is required today").

**Real-world verification still required (LAN + physical NVR/Camera 7 required)**:
- [ ] Open `p3.syn`
- [ ] Connect to the real NVR
- [ ] Start Live Runtime
- [ ] Open Command Center → Live CCTV
- [ ] Confirm Camera 7's tile is fully visible
- [ ] Confirm scrolling works correctly
- [ ] Confirm the full camera frame is visible (no clipping)
- [ ] Confirm live video continues updating
- [ ] Confirm the Digital Twin remains unaffected

Do NOT mark this complete until every checkbox above is verified against the real Camera 7 stream.
...
### Session: Live CCTV Dashboard (Command Center)

**Date**: 2026-08-04, immediately following Live Camera Viewer V1 — same development arc, now with Camera 1's physical RTSP stream confirmed reachable (an NVR-side channel/signal issue on the previous session's attempt, not a code defect; resolved once Channel 1 was confirmed showing live video on the NVR's own monitor).

**Subsystem**: `command_center` (new panel + new gateway), `live_camera_pipeline` (additive), `live_runtime` composition root (additive).

**Context**: a professional, operator-facing, multi-camera CCTV monitoring grid in Command Center — the app already documented as SynEvac's live/replay monitoring surface — reusing the exact production RTSP pipeline verified working this session, with YOLO detection overlays reusing already-computed tracking results (no second detector), and a grid that scales dynamically with however many cameras are actually configured (never hardcoded to a fixed count).

**Files created**: `command_center/live_camera_view_gateway.py` (`LiveCameraViewGateway` + `CameraTileData` — the read-only counterpart to `live_operator_action_gateway.py`, deliberately excluded by omission from `tests/test_live_command_center.py::CommandCenterLiveIntegrationGuardTests._LIVE_INTEGRATION_FILES`, the same mechanism that gateway already relies on), `command_center/camera_tile_widget.py` (`CameraTileWidget` — one grid cell: video + bounding-box/track-ID/confidence overlay drawn via `QPainter` on the full-resolution `QImage` before scaling, resolution/FPS labels, "Not Configured" placeholder), `command_center/live_camera_grid_panel.py` (`LiveCameraGridPanel` — `QGridLayout`, column count `ceil(sqrt(tile_count))` recomputed every refresh, tiles rebuilt only when the camera_id *set* changes), `tests/test_live_camera_view_gateway.py`, `tests/test_command_center_live_camera_grid_panel.py`.

**Files modified**: `live_camera_pipeline/pipeline.py` (`LiveCameraPipeline` gained a second single-slot cache, `_latest_detections`/`latest_detections(camera_id)`, populated in `run_cycle()`'s existing loop right after the tracker branch already resolves `raw` — same "no second read/detect" discipline `_latest_frames`/`latest_frame()` established one milestone ago), `live_runtime/factory.py` (constructs `camera_view_gateway = LiveCameraViewGateway(camera_manager, camera_pipeline, frame_sources)` alongside `operator_action_gateway`, `None` whenever no `CameraManager` was supplied), `live_runtime/runtime.py` (`LiveRuntime` gained `camera_view_gateway=None`, same untyped-attribute convention as every other gateway), `live_runtime_launcher/session.py` (`open_command_center()` forwards the third argument), `command_center/main_window.py` (`enable_live_mode()` gained `camera_gateway=None`, forwarded to Dashboard), `command_center/dashboard.py` (new "Live CCTV" tab, `set_camera_gateway()`, `show_live()` called from `apply_snapshot()`), `tests/test_live_camera_pipeline.py` (new `LiveCameraPipelineLatestDetectionsCacheTests`).

**Architectural decisions**: `command_center/` is mechanically forbidden (`CommandCenterLiveIntegrationGuardTests`) from importing `camera_manager.manager`/`live_camera_pipeline` in any of its live-integration files (`dashboard.py`, `main_window.py`, etc.) — the new gateway file is the one place allowed to, by never being added to that guard's own file list, exactly mirroring how `live_operator_action_gateway.py` already achieves the same exclusion. `CameraTileData`'s `frame`/`detections` fields stay untyped `Any` (never importing `CameraFrame`/`RawHumanDetection` outside the gateway itself) — the same "typed Any, not imported for its own sake" convention every `CommandCenterSnapshot` passthrough field already follows. Bounding-box coordinates were traced to `UltralyticsYOLOBackend.infer()`'s own `box.xyxy` and confirmed absolute pixel-space `(x1,y1,x2,y2)` in the original frame — overlay boxes are drawn on the full-resolution `QImage` before any scaling, not re-derived against a scale factor. FPS is a small, honest, locally-derived rolling average over consecutive `CameraFrame.timestamp` values the tile has already been handed — never a second read/decode.

**Tests added**: 205 targeted tests run and passing across every touched/new file — `tests/test_live_camera_pipeline.py` (new detection-cache tests + pre-existing frame-cache tests), `tests/test_live_camera_view_gateway.py` (new, 6 tests), `tests/test_command_center_live_camera_grid_panel.py` (new, 6 tests), `tests/test_live_command_center.py` (guard + integration tests, unmodified, still passing), `tests/test_command_center.py`, `tests/test_live_runtime.py`, `tests/test_live_runtime_architecture_guards.py`, `tests/test_application_live_runtime_launcher.py`, `tests/test_no_cv_dependencies.py`, `tests/test_designer_live_camera_view_panel.py`. **Full-suite run**: attempted, did not complete — see Known Issues below.

**Real Camera 1 hardware verification — succeeded this session** (the environmental block from the previous Live Camera Viewer V1 session was resolved: Channel 1 confirmed showing live video on the NVR's own monitor first, then production pipeline reconnected successfully). A real `LiveRuntimeSession(ApplicationMode.LIVE, human_detector_weights_path=...)` was constructed against the real Camera 1 config (real `LocalFileCredentialStore`, real `RTSPFrameSource`, real `OpenCVFrameDecoderBackend`, real YOLO weights), driven through real `run_cycle()`s, and Command Center's new Live CCTV tab was screenshotted: Camera 1's tile showed genuine live decoded video (1280x720, visible building lobby/stairwell, live burned-in timestamp matching the moment of capture) with status `Online`; Cameras 2–4 (present in the Building but never RTSP-configured) correctly rendered `Not Configured`, in a grid that auto-sized to 2x2 for the 4 total cameras. `frame_sources` contained exactly one entry (Camera 1's id) — confirming exactly one RTSP connection was opened, no duplicate/second decode path anywhere. No people were in frame at capture time, so 0 detections/no overlay boxes were drawn — correct, honest behavior, not a defect.

**Known issues**: **Full-suite pytest run did not complete** — left running in the background for ~59 minutes, accumulated only ~3.75s of CPU time (confirmed hang, not a slow run), killed without further investigation per explicit scope instruction. No hung-test name recoverable (stdout was fully buffered, nothing flushed before the kill). Most likely a third, not-yet-catalogued instance of the same unmocked-Qt-modal-dialog class of bug as the two already-known flaky Builder tests (see Known Flaky Tests above) — next session should re-run with `python -u` (unbuffered) and `-x -v` to catch exactly where it stalls. This milestone's own 205 targeted tests all pass in seconds and show no sign of this behavior, so the hang is very unlikely to be caused by anything built this session.

---

### Session: Digital Twin Live Occupant Marker (Camera 7, real laboratory building)

**Date**: 2026-08-04/05, immediately following Live CCTV Dashboard — first milestone against the real laboratory building (`p3.syn`), superseding an earlier, explicitly-discarded investigation against an older imaginary building (p6.syn, then briefly p5.syn) per direct user correction: "Start the investigation again from scratch" against p3.syn.

**Subsystem**: `command_center/building_view.py` (additive — the existing Digital Twin rendering surface; per standing project convention, Building Designer/Command Center's `BuildingView` *is* the digital twin, no separate package was created), `command_center/dashboard.py` (one-line wiring), `p3.syn` (Camera 7 reconfigured from Simulation to Live), credential store (new entry).

**Context**: Camera `ffbdac49-7982-4b61-86cc-98cbbc73b899` ("Camera 7", Ground Floor) is the one physical camera mapped to real NVR Channel 1 for this entire project — explicit instruction: "Do not guess any other mapping." Goal: when this camera's real pipeline detects a person, show an anonymous live marker (Track ID only) on the Digital Twin — appearing only from real detections, disappearing when the runtime loses the occupant, updating live, zero mock data. Explicitly excluded from this milestone: world-coordinate estimation, homography, triangulation, cross-camera identity, occupancy heatmaps, evacuation routing.

**Files modified**: `command_center/building_view.py` — new module-level `estimate_display_position(occupant, floor)` (currently returns the occupant's associated camera's own configured position; deliberately encapsulated as a standalone function, not inlined, so a future milestone can add camera rotation/FOV/projection math *without changing the renderer or any caller*), `_find_camera_on_floor(floor, camera_id)`, `_LIVE_MARKER_STATUSES = (OccupantStatus.NEW, OccupantStatus.ACTIVE)` (deliberately excludes `TEMPORARILY_LOST`/`EXITED`/`EXPIRED` — a marker must vanish the moment the runtime itself stops actively tracking the occupant, not persist through a lost-track grace period), new `BuildingView.set_live_occupants(snapshot)` / `_render_live_occupants(floor)` / `_live_occupant_tooltip(occupant)`. `command_center/dashboard.py` — one line in `apply_snapshot()`: `self.building_view.set_live_occupants(snapshot.live_occupants)`. `p3.syn` — Camera 7's block changed `mode: "Simulation"` → `"Live"` with a real `connection` block (`rtsp_address`, `username: synevac_svc`, `credential_ref` set to Camera 7's own camera id — explicitly NOT reusing the older p5.syn camera's credential entry). `~/.synevac/credentials.json` — new entry added for Camera 7's `credential_ref`, password copied from the existing validated entry via a script that never printed or logged the password value itself.

**Architectural decisions**: `LiveOccupantsSnapshot`/`live_occupants` (already the canonical live-occupancy source of truth from an earlier milestone) is the sole data source — no new occupancy tracking, no duplicate state. Marker placement is a pure function of `(occupant, floor)`, resolvable to "camera's own configured position" today with zero information about camera orientation — the explicit design bet is that adding real projection math later is a change *inside* `estimate_display_position()` only, never a renderer/caller change.

**Tests added**: 11 (`tests/test_command_center_live_digital_twin.py`), all passing. Confirmed via this session's regression run (101 passed, see Live CCTV Layout Fix entry below, includes this file).

**Real Camera 7 hardware verification — succeeded this session**: real `LiveRuntimeSession` constructed against Camera 7's real config (real credential store entry, real `RTSPFrameSource`, real `OpenCVFrameDecoderBackend`), driven through real `run_cycle()`s against p3.syn, Digital Twin floor set to Ground Floor, screenshot captured (`C:\Users\riddh\.claude\jobs\99e9d6ab\tmp\p3_digital_twin_final_screenshot.png`) showing the app live and ready; no person was walking in front of the physical camera at the exact capture moment, so the screenshot itself does not show a marker mid-render, but the full pipeline (connection → detection → occupant snapshot → `set_live_occupants()` → render) was proven live and crash-free end-to-end.

**Known issues**: none new in this milestone's own code. An intermittent Live Runtime startup hang was investigated in a separate, explicitly bounded side-investigation during this arc (see Known Issues in Quick Snapshot above and the preserved instrumentation) — confirmed unrelated to and not caused by this milestone's changes, left undiagnosed per explicit user instruction to stop after two clean bounded attempts.

---

### Session: Live CCTV Layout Fix (Command Center)

**Date**: 2026-08-05, immediately following the Digital Twin Live Occupant Marker milestone, same development arc.

**Subsystem**: `command_center/live_camera_grid_panel.py` (additive), `tests/test_command_center_live_camera_grid_panel.py`.

**Context**: real-GUI testing against p3.syn (32 real cameras) surfaced Camera 7's Live CCTV tile appearing "vertically collapsed" — only the top ~10% of the frame visible. A dedicated, scoped investigation (RTSP/OpenCV/YOLO/tracking/Digital Twin explicitly excluded from suspicion per direct instruction) instrumented `CameraTileWidget`'s full geometry pipeline live (widget size, `QLabel` size, `QPixmap` size before/after `.scaled()`) and proved the scaling math itself was exact and correct at every measured point (1280x720 source → 244x137 displayed, precise aspect-ratio-preserving scale, log preserved at `C:\Users\riddh\.claude\jobs\99e9d6ab\tmp\diagnose_tile_geometry.log`). Root cause was one layer up: `LiveCameraGridPanel` had no `QScrollArea` — its natural size for 32 tiles in a `ceil(sqrt(32))=6`-column grid (~1540x1607px, confirmed via the same live instrumentation) exceeds the ~900px-tall Command Center window, and Qt clips a widget larger than its parent rather than scrolling to it by default.

**Design comparison presented to and approved by the user before implementation**: Option A (wrap the existing grid in a `QScrollArea`) vs. Option B (pagination) vs. Option C (selectable fixed layouts: 1/2x2/3x3/4x4/6x6) vs. Option D (other). Option A was recommended and chosen: smallest diff (one new widget wrapper, zero changes to `CameraTileWidget` or the grid's own `ceil(sqrt(n))` column math), no new UI-state to build/persist (unlike B/C), and scales to any future camera count (50+) with zero further code changes — scrolling further simply replaces clipping further, nothing else about tile rendering, aspect ratio, or frame quality changes.

**Files modified**: `command_center/live_camera_grid_panel.py` — `QScrollArea` added to the `PyQt6.QtWidgets` import; `__init__` now wraps the existing `_grid_container` in `self._scroll_area = QScrollArea()` (`setWidgetResizable(True)`, `setWidget(self._grid_container)`), added to the outer layout in place of the old direct `_grid_container` add, initially hidden; both branches of `show_live()` (`gateway is None` and the `has_cameras` branch) now toggle `_scroll_area.setVisible(...)` instead of `_grid_container.setVisible(...)`. `_rebuild_grid()` and the column-count math are byte-for-byte unchanged. `tests/test_command_center_live_camera_grid_panel.py` — existing `_grid_container.isHidden()` assertions updated to `_scroll_area.isHidden()` (the container itself is never hidden/shown directly anymore, only its new scroll-area wrapper is); new `LiveCameraGridPanelOverflowTests` class added with 2 tests: `test_scroll_area_hosts_the_grid_container_and_is_resizable` (structural — `scroll_area.widget() is grid_container`, `widgetResizable() is True`) and `test_many_tiles_do_not_crash_and_grid_container_may_exceed_the_scroll_areas_own_size` (the actual regression guard — 32 tiles against a deliberately small `QScrollArea`, confirms the content widget's `sizeHint()` height is allowed to exceed the scroll area's own height, i.e. scrolling is structurally possible instead of Qt clipping the excess).

**Architectural decisions**: `setWidgetResizable(True)` lets the scroll area's *viewport* track the panel's own available space while the *content* (`_grid_container`) keeps whatever size its own layout naturally computes — the reverse (letting content shrink to fit) is what caused the original clipping. No change to `CameraTileWidget`, tile minimum size, aspect ratio handling, or frame-scaling logic anywhere, per explicit instruction.

**Tests added**: 2 new (`LiveCameraGridPanelOverflowTests`). **Tests passed**: 8/8 in `tests/test_command_center_live_camera_grid_panel.py`; 101/101 in the broader regression run (`tests/test_command_center.py` + `tests/test_live_command_center.py` + `tests/test_command_center_live_digital_twin.py`), zero regressions. A full-suite `pytest` run was also attempted this session and hit the same pre-existing, already-documented Qt-modal hang (see Known Issues in Quick Snapshot) — not caused by this change (the same hang was already documented one milestone prior, before this file was touched), not investigated further per the same established scope boundary.

**Known issues**: **Real-GUI, real-LAN verification against Camera 7 (p3.syn) is deferred to next session** — network/NVR access was unavailable at the point this milestone reached its verification step. Everything verifiable without the physical camera (unit/regression tests, structural scroll-area wiring) is complete and passing; the one remaining item is a real screenshot/observation confirming Camera 7's tile is fully visible and scrollable, live video keeps updating, and the Digital Twin tab is visually unaffected — mechanically expected to pass given the root-cause diagnosis and targeted tests, but not yet empirically confirmed on-screen.

---

### Session: Branch Merge — Live CCTV/Digital Twin arc + Camera->Zone Assignment/Inventory arc combined into main

**Date**: 2026-08-06

**Context**: two independent development arcs existed side by side: (a) Live CCTV Dashboard -> Digital Twin Live Occupant Marker -> Live CCTV Layout Fix, developed directly in the main checkout as uncommitted working-tree changes across several prior sessions; (b) Camera -> Zone Assignment V1 -> System Capability Inventory, developed in an isolated git worktree/branch (`milestone-camera-zone-assignment`) this session, based on the same commit both arcs started from. The user asked to get the System Capability Inventory doc into the main checkout, which meant reconciling both arcs.

**What happened**: arc (a)'s uncommitted changes were safely stashed (`git stash push -u`), arc (b)'s branch was fast-forward merged into `main` (clean, no conflicts — the two arcs touched no files in common except `PROJECT_STATE.md`), then the stash was reapplied. The only conflict was in `PROJECT_STATE.md` itself (Quick Snapshot + Session Changelog sections, both arcs had appended their own milestone entries). Resolved by combining both sides in full — no content from either arc was dropped, only reordered into correct chronological order (arc (a)'s three sessions, dated 2026-08-04/05, precede arc (b)'s two sessions, dated 2026-08-06) and the Quick Snapshot synthesized to reflect the combined, current state.

**Known issues carried forward from arc (a)** (real, unresolved, now part of `main`'s own history — see Quick Snapshot above for full detail): Open Verification Queue #1 (Live CCTV 32-camera layout fix, real-LAN verification still pending), a recurring full-suite Qt-modal-hang pattern, and an unresolved intermittent Live Runtime startup hang.
...
**Known issues**: none new. The two not-yet-merged branches from this development arc (`worktree-building-camera-topology-design` for the Building/Camera Topology investigation doc, and this branch `milestone-camera-zone-assignment` for the Camera -> Zone Assignment milestone + this inventory) still need a merge decision — out of scope for this milestone, noted for whoever manages branch integration next. **Update**: `milestone-camera-zone-assignment` (Camera -> Zone Assignment V1 + this inventory) was merged into `main` later the same day, alongside the previously-uncommitted Live CCTV Dashboard/Digital Twin/Layout Fix arc — see the "Branch Merge" session entry above and the Multi-Camera Streaming Architecture entry below. `worktree-building-camera-topology-design` remains unmerged.

---

### Session: Multi-Camera Streaming Architecture (Phase 1)

**Date**: 2026-08-06, immediately following the System Capability Inventory and the `main` merge described above. Developed on a fresh branch/worktree (`milestone-multi-camera-streaming`) cut from `main` *after* first checkpointing the previously-uncommitted Live CCTV Dashboard/Digital Twin/Layout Fix arc into its own commit (per explicit user decision — see that commit's own message), since this milestone needed to build directly on `command_center/live_camera_grid_panel.py`/`camera_tile_widget.py`/`live_camera_view_gateway.py`.

**Context**: the user asked for the production architecture behind simultaneous multi-camera live viewing — explicitly *not* about any one camera/NVR, no LAN/RTSP/decoder/YOLO/Digital-Twin/cross-camera-identity work, "only the streaming architecture," required to scale 1 → 4 → 16 → 32 → 100+ cameras without redesign. Required investigating and reporting on ownership (RTSP streams, decoded frames, frame cache) before writing any code, per `PROJECT_STATE.md`/`docs/architecture/system_capability_inventory.md` as source of truth.

**Investigation findings (the headline result)**: the production architecture was **already** correctly designed for N simultaneous cameras, end to end, before this milestone touched anything:
1. **Who owns every RTSP stream**: `live_camera_pipeline/rtsp_frame_source.py::RTSPFrameSource` — one instance per camera, each with its own decoder/connection/retry state. Constructed **exclusively** by `live_runtime_launcher/rtsp_camera_sources.py::build_rtsp_frame_sources()`, which already loops every qualifying `Camera` on every floor (confirmed the **only** production call site by repo-wide grep — every other hit is in `scripts/`/`tests/`).
2. **Who owns every decoded frame**: `live_camera_pipeline/pipeline.py::LiveCameraPipeline` — **one shared instance** for the whole session (its own comment: *"not one per camera"*), never one per camera.
3. **Where the frame cache is**: `LiveCameraPipeline._latest_frames: Dict[str, CameraFrame]` — already per-camera-dict-keyed, plus a sibling `_latest_detections` dict, both single-slot "last known good" per camera.
4/5. **Which object represents one live camera, and which should**: no single unifying class exists today, and none is needed — the concept is correctly distributed across `Camera` (config) → `RTSPFrameSource` (connection) → `CameraStatus`/`CameraConnectionState` (status) → `CameraFrame` (latest frame) → `command_center/live_camera_view_gateway.py::CameraTileData` (the per-refresh presentation combination of all of the above). Introducing a new "LiveCamera" domain class would have duplicated `CameraTileData`'s already-correct job.
6. **Can the existing architecture already support multiple simultaneous cameras**: **yes** — `CameraManager` (dict of cameras), `LiveCameraViewGateway.camera_tiles()` (one tile per camera in `CameraManager.all_cameras()`), and `LiveCameraGridPanel` (dynamic `ceil(√n)` grid, rebuilds only when the camera *set* changes, `QScrollArea`-wrapped, already tested at 32 tiles from the prior Layout Fix milestone) were all already correctly multi-camera before this milestone.
7. **Every "assumes one camera" assumption found**: exactly one, and it is deliberate/disclosed/out of scope — `designer/widgets/live_camera_view_panel.py` + `MainWindow._refresh_live_camera_view_panel()` (`next(iter(frame_sources))`, own comment: *"V1 shows exactly one camera... a channel selector is out of scope"*). This is Designer's own small single-camera preview widget, a different, intentionally-scoped feature from Command Center's grid — not a bug, not touched.

**Conclusion drawn from the investigation**: this was a **verification and light-hardening** milestone, not a from-scratch design/build.

**Files modified**: `command_center/camera_tile_widget.py` — added `_format_timestamp()` (renders the frame's own already-existing `CameraFrame.timestamp`, previously used only internally for the FPS rolling average, as an `HH:MM:SS` string) and included it in the per-tile detail label alongside resolution/FPS/detection-count — the one genuine gap found against the milestone's own explicit "Frame timestamp available" requirement. No other production file was changed — ownership, caching, and grid-building logic were already correct.

**Files created**:
- `tests/test_camera_tile_widget.py` — `CameraTileWidget` had no dedicated test file before this milestone (only exercised indirectly via grid-panel tests using frame-less fakes); 8 new tests cover real-frame rendering, the new timestamp display (including a malformed-timestamp fallback-to-"-" case), FPS derivation, and detection-count display.
- `tests/test_rtsp_single_ownership_architecture_guard.py` — a repo-wide regex-scan guard (same convention as `tests/test_no_cv_dependencies.py`) proving `RTSPFrameSource(` is constructed in exactly one production file; a second guard confirms that one file still genuinely constructs it (so the first guard can't silently pass by going stale). Locks in "no duplicate stream ownership" as a standing regression check, not just a point-in-time finding.
- `tests/test_multi_camera_streaming_e2e.py` — the milestone's central deliverable: builds the **real** production spine (`Building` with N `LIVE` cameras → `build_rtsp_frame_sources()` → `build_live_runtime()` → `CameraManager`/`LiveCameraPipeline`/`LiveCameraViewGateway` → `LiveCameraGridPanel`) at N = 1, 4, 16, 32, and 100, with zero LAN/real-network dependency (`FakeRTSPBackend`/`MockHumanDetector`, the same offline doubles `tests/test_rtsp_camera_sources.py`/`tests/test_live_camera_pipeline.py` already establish), asserting exactly N distinct `RTSPFrameSource` objects (with distinct decoders), exactly one shared `LiveCameraPipeline`, exactly N `CameraTileData` tiles, and exactly N grid tiles — end to end, in under 7 seconds even at N=100. Plus two smaller regression tests (no frame source ever shared across two camera ids; shrinking the Building's camera set never leaves a stale source).

**Architectural decisions**: none required — explicitly followed the milestone's own "preserve existing ownership, do not duplicate RTSPFrameSource/CameraManager/LiveCameraPipeline/LiveRuntime/Gateway architecture" instruction by finding that no duplication was needed in the first place. The only new code is a UI-presentation addition (timestamp display) and three test files.

**Tests added**: 8 (`test_camera_tile_widget.py`) + 2 (`test_rtsp_single_ownership_architecture_guard.py`) + 3, one parametrized across 5 camera counts via `subTest` (`test_multi_camera_streaming_e2e.py`) = 13 new test methods, 17 total assertions-runs counting subtests. **Tests passed**: 380 passed, 0 failed, 5 subtests passed, across the full targeted batch (every RTSP/camera-manager/live-camera-pipeline/live-runtime/command-center/grid-panel/tile-widget test file, plus the three new files).

**Known issues**: none new. Same pre-existing, environment-only gap rediscovered from the Camera -> Zone Assignment milestone: this fresh worktree was missing the gitignored `weights/yolov8n.pt` binary (copied in from the main checkout, no repository change — `weights/*.pt` stays gitignored). Real-LAN verification of the Live CCTV layout/grid against the actual Camera 7 stream remains open (Open Verification Queue #1, above) — explicitly out of scope for this milestone ("No LAN verification is required today").

**Explicitly not started, per direct instruction**: Camera Topology, Coverage, Virtual Zones, Cross-camera identity, World-coordinate estimation, Digital Twin enhancements. These remain exactly where the System Capability Inventory (§23) and Building Topology investigation (`docs/architecture/building_camera_topology_design.md`) left them.
