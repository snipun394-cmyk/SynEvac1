...
- **Current Phase**: System Capability Inventory Complete (documentation-only milestone, no code changed) — most recent in a run of five milestones combined into `main` on 2026-08-06: Live CCTV Dashboard → Digital Twin Live Occupant Marker → Live CCTV Layout Fix → Camera -> Zone Assignment V1 → System Capability Inventory
- **Current Version**: v0.9.5
- **Last Completed Milestone**: SynEvac Capability & Architecture Inventory — a full, verified, class-by-class inventory of ~20 major subsystems, dependency graphs, and a capability matrix (see `docs/architecture/system_capability_inventory.md`). Immediately preceded by Camera -> Zone Assignment V1 (Camera.zone_ids now genuinely multi-assignable and authorable in Studio). Both were developed on a separate branch/worktree from an earlier, independent arc against the real laboratory building (`p3.syn`, Camera 7): Live CCTV Dashboard (multi-camera grid + YOLO overlay in Command Center), Digital Twin Live Occupant Marker (Camera 7's real detections rendered on BuildingView), and Live CCTV Layout Fix (QScrollArea fix for 32-camera tile clipping) — that arc existed only as uncommitted working-tree changes until this merge. All five are now combined in `main`'s history; see the Session Changelog below for each in full detail.
- **Next Planned Milestone**: TBD. Two concrete threads are open: (1) **Open Verification Queue #1** below — real-LAN/NVR confirmation of the Live CCTV 32-camera layout fix against Camera 7, blocked on physical access, not yet closed; (2) the System Capability Inventory's own §23 ("What Future Milestones Naturally Build Upon") — closing the perception dormancy chain (YOLO/Tracking/Cross-Camera-Identity/Live-Occupants are one config parameter away from activation, per the inventory), Camera Topology activation (unblocked by Camera -> Zone Assignment V1), and camera-to-camera overlap (already designed in `docs/architecture/building_camera_topology_design.md`).
...
- **Total Tests**: not yet re-verified as one combined number. This merge (2026-08-06) combines two previously-separate, independently-tested change sets that were never run together as one suite until now: (a) Live CCTV Dashboard/Digital Twin/Layout Fix — 205+11+8 targeted tests passing, full-suite blocked by a hang on every attempt (see Known Issues); (b) Camera -> Zone Assignment V1/System Capability Inventory — ~5967 passing across all 430 pre-merge test files, run in an isolated worktree that did not yet contain the Live CCTV Dashboard/Digital Twin/Layout Fix files. **Re-running the full suite once against the merged `main` should be the first action of the next session.**
- **Known Flaky Tests**:
