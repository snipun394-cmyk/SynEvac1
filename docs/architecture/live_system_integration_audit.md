...
- `Advisory → Live Command Center` — **still not built** — `StateManager.advisory_report`/`latest_advisory_report()` exist but no Command Center panel or Designer debug view reads them yet; unchanged from §16's own note on this point.
