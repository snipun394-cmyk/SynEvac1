...
**HISTORICAL / SUPERSEDED.** This audit's own central finding — that no production composition root wires the platform's subsystems into a running cycle — was resolved by `live_runtime/factory.py::build_live_runtime()` and later made reachable from the actual application by the Application Live Runtime Launcher milestone (`docs/architecture/application_live_runtime_integration.md`). The `live_system.integration` gateway generation this audit proposes/discusses below was one path explored on the way there; it was never wired into production, was fully superseded by the newer `live_system.building_state_gateway`/`live_ai_gateway`/`live_advisory_gateway`/`Engine*Gateway` generation, and was trimmed down to legacy-only extension-point typing by the Live Runtime Architecture Cleanup milestone (`docs/architecture/live_runtime_architecture_cleanup.md`). Left unmodified below as a historical record of the investigation, not as a description of current architecture.
...
## 16. Milestone 4 — Live AI Inference Runtime Integration (COMPLETED — `BuildingState → AI Inference` now IMPLEMENTED)

Status: **`LiveOrchestrator.run_cycle()` now produces a real `LiveAIPredictionSnapshot` from canonical `BuildingState` every cycle a `live_ai_gateway` is configured.** Full detail: `docs/architecture/live_ai_runtime_integration.md`.

`live_system.live_ai_gateway` (new module) adds `LiveAIInferenceGateway` (the injected Protocol `LiveOrchestrator` now holds, alongside — never replacing — `building_state_gateway`), `RegistryLiveAIInferenceGateway` (the real adapter, composing an already-built `ai_registry.LiveAIInferenceService`), `ThrottledLiveAIInferenceGateway` (an optional, unused-by-default interval wrapper), and `LiveAIPredictionSnapshot`/`AISystemStatus`. `StateManager`/`LiveBuildingSnapshot` gained one additive field (`ai_prediction_snapshot`) plus `update_ai_prediction()`/`latest_ai_prediction()`, mirroring the `building_state` field's own Milestone 1 pattern exactly — the pre-existing `ai_predictions`/`ai_inference_gateway` seam (unrelated, still unimplemented in production) is untouched.

**`BuildingState → AI Inference`: now ✅ IMPLEMENTED** (supersedes the "❌ not yet built" status this arrow carried since Milestone 2). Only `BottleneckOccurrenceModel_LiveCompatible` (`PRODUCTION_CANDIDATE`) is treated as an operational signal; `EvacuationTimeModel_LiveCompatible` (`EXPERIMENTAL`) is exposed only under the field name `evacuation_time_experimental` and can never affect `system_status` on its own. Every failure mode (no model, incompatible model, corrupted model, missing `BuildingState`) is caught and turned into an honest status, never a crash and never a fabricated prediction — proven directly against a full `LiveOrchestrator.run_cycle()`.

**Still explicitly NOT connected, unchanged from Milestone 3:**
- `AI Inference → Decision Policy` — **not built.** `decision_policy/` never imports `live_system.live_ai_gateway`/`ai_registry`/`ai_features` (mechanically enforced).
- `AI Inference → Advisory System` — **not built at this milestone.** (Superseded below — Milestone 5 builds this connection, deliberately routed around Decision Policy rather than through it; see §17.)
- `Advisory → Live Command Center` — **still not built**, and this milestone's own investigation (Phase 12) confirmed *why* wiring even debug-level AI status into Designer/Command Center right now would be premature: neither has a `ModelRegistry`/trained-model-loading path today, and building one just for a debug label would itself be exactly the "fake live data" this phase's own instructions warn against. Deferred to the Live Command Center milestone.
- No building control execution, no automatic voice broadcast were introduced — mechanically enforced.

## 17. Milestone 5 — AI-Augmented Decision Policy & Advisory Integration (COMPLETED — `AI Inference → Advisory System` now IMPLEMENTED; `AI Inference → Decision Policy` confirmed to remain, deliberately, NOT built)

Status: **the validated Bottleneck Occurrence AI prediction now flows into `advisory_system` as a supporting predictive signal, every live cycle a `live_advisory_gateway` is configured.** Full detail: `docs/architecture/ai_augmented_advisory_integration.md`.

This milestone re-examined §6's original finding ("Advisory System has never been wired to any live state, in any form") and §5/§16's "`AI Inference → Decision Policy` — not built" line, and reached a deliberate, investigated conclusion rather than simply filling in both arrows the illustrative diagram in §10 implies: **`decision_policy.generate_policy()`'s six rule modules read exclusively from `GroundTruth`'s own post-hoc, completed-simulation analytics, confirmed field-by-field to have no `BuildingState` equivalent** (`docs/architecture/ai_augmented_advisory_integration.md` §1) — the same structural incompatibility §5 already found for AI Inference's own training features, now shown to apply equally to Decision Policy's inputs. Rather than fabricate a `GroundTruth`-shaped stand-in from `BuildingState` to force the illustrative `BuildingState → AI Inference → Decision Policy → Advisory System` ordering to compile, this milestone routes AI evidence directly into `advisory_system` instead — the one package whose own pre-existing design (`AdvisoryInputs.ai_predictions`/`.ai_confidence`, already noted in §6 as "a real asset for future live wiring") was already built to tolerate exactly this.

**What was built:** `advisory_system.ai_evidence.AIDecisionEvidence` (new module) — a plain, frozen, building-wide-only record (`bottleneck_occurrence_probability`, `bottleneck_predicted`, `threshold`, `model_id`/`model_version`/`model_status`, provenance timestamps; deliberately **no** localization field — §2 of the new architecture doc). `live_system.live_advisory_gateway` (new module) — `LiveAdvisoryGateway` (the injected Protocol, mirroring `live_ai_gateway`'s own shape), `ai_decision_evidence_from_prediction_snapshot()` (the pure `LiveAIPredictionSnapshot → AIDecisionEvidence` converter), and `ReplayCompatibleAdvisoryGateway` — named honestly, not pretending to run from `BuildingState` alone (§12 of the new doc): it requires a caller to supply a real `Building`/`Scenario`/`GroundTruth` (a completed campaign or Replay session), exactly mirroring `command_center/incident_data.py`'s own established per-frame `zone_policy`/`exit_policy`/`stair_policy`/`announcement_policy` recomputation pattern, while genuinely refreshing `AIDecisionEvidence` from a live `BuildingState` every cycle. `advisory_system.advisory_engine.py` gained one new optional input (`AdvisoryInputs.ai_decision_evidence`) read at exactly four points — never touching `SHELTER_IN_PLACE`/`EVACUATE_IMMEDIATELY` zone confidence, never overriding an `AVOID`/`CLOSE` decision, only ever strengthening an already-independently-decided `WAIT` zone's confidence, annotating firefighter/commander confidence, and adding one additive, building-wide-only `"Monitor for Building-Wide Congestion"` recommendation. `live_system.state_manager.LiveBuildingSnapshot` gained one additive field (`advisory_report`) plus `update_advisory_report()`/`latest_advisory_report()`, mirroring the `ai_prediction_snapshot` field's own Milestone 4 pattern exactly. `LiveOrchestrator.run_cycle()` gained one new optional stage, inserted immediately after live AI inference: `LiveAIPredictionSnapshot → AIDecisionEvidence → AdvisoryReport`.

**`AI Inference → Advisory System`: now ✅ IMPLEMENTED** (supersedes the "❌ not yet built" status this arrow carried in §16). **`AI Inference → Decision Policy`: remains ❌ NOT built — confirmed, investigated, and intentional**, not an oversight: `decision_policy/` still never imports `live_system.live_ai_gateway`/`live_advisory_gateway`/`ai_registry`/`ai_features`/`advisory_system` (mechanically enforced by the pre-existing guards in `tests/test_live_ai_runtime_integration.py` plus this milestone's own `tests/test_ai_augmented_advisory.py::NoOutputExecutionBoundaryTests`), and no file this milestone added imports `decision_policy` either. AI cannot influence `AVOID`/`CLOSE`/`SHELTER_IN_PLACE` decisions because it never reaches the code that makes them — the strongest available safety guarantee, enforced by absence of a code path rather than by a rule that could be bypassed.

**Still explicitly NOT connected, unchanged or newly confirmed by this milestone:**
- `AI Inference → Decision Policy` — confirmed, investigated, remains not built (above).
- `Advisory → Live Voice Evacuation` — **still not built.** `live_advisory_gateway.py` and every file in `advisory_system/` are mechanically confirmed to never import `voice_evacuation`/`speaker_manager` (`tests/test_live_system.py::LiveSystemPackageDependencyDirectionTests`, extended this milestone by `tests/test_ai_augmented_advisory.py::NoOutputExecutionBoundaryTests`). `AdvisoryReport` is stored in `StateManager` and an `EventType.ADVISORY_REPORT_UPDATED` event is emitted, but nothing subscribes to that event to trigger a broadcast.
- `Advisory → Live Building Control` — **still not built**, same guard, against `building_control.controller`/`building_control.providers`.
- `Advisory → Live Command Center` — **not built at this milestone.** (Superseded below — Milestone 6 builds this connection; see §18.)
- No building control execution, no automatic voice broadcast, no firefighter command authority were introduced — mechanically enforced (`docs/architecture/ai_augmented_advisory_integration.md` §14).

## 18. Milestone 6 — Live Command Center Integration (COMPLETED — `BuildingState/AI/Advisory → Command Center` now IMPLEMENTED; `Advisory → Voice/Building Control Execution` confirmed to remain, deliberately, NOT built)

Status: **Command Center is now a dual-mode operational interface, rendering either a Replay session (unchanged) or a real-time view of `LiveOrchestrator`/`StateManager`'s current state, through one data-source abstraction.** Full detail: `docs/architecture/live_command_center_integration.md`.

This milestone closes the one arrow §9's Live Integration Matrix marked "❌ every panel driven exclusively by the replay `QTimer`; `DashboardCommandCenterGateway` is real but has zero production callers" — not by retrofitting that old, test-only gateway (which reads only `LiveBuildingSnapshot.building_observation`, ignoring the canonical `building_state`/`ai_prediction_snapshot`/`advisory_report` fields three later milestones added), but by building a new, richer `LiveCommandCenterDataSource` (`live_system/live_command_center_gateway.py`) that reads exactly `StateManager.current().building_state`/`.ai_prediction_snapshot`/`.advisory_report`/`.component_timestamps` and nothing else — never instantiating `BuildingStateEstimator`, `LiveAIInferenceService`, or `AdvisoryOrchestrator` itself. A new `command_center.data_source.CommandCenterSnapshot` presentation type (§2/§3 of the new architecture doc) normalizes both Replay and Live into one shape `Dashboard` already knows how to render — reusing `IncidentFrame` (via a new `frame_from_building_state()` adapter) and the real, unmodified `advisory_system.AdvisoryReport` rather than inventing a second state model.

**`BuildingState → Command Center`, `AI Inference → Command Center`, `Advisory → Command Center`: now ✅ IMPLEMENTED.** `Dashboard.apply_snapshot()` is the one Live render call; `MainWindow.enable_live_mode(data_source)` plus a ~1 Hz `QTimer` (`live_refresh_timer`, matching `LiveOrchestrator`'s own default cycle interval) drive it. Four pre-existing panels (`CivilianAnnouncementsPanel`/`FirefighterIntelligencePanel`/`BuildingRecommendationsPanel`/`CommanderSummaryPanel`) needed **zero changes** — they already read only `(frame, report)`. `BuildingView` needed **zero changes** — `frame_from_building_state()` produces the same `IncidentFrame` type Replay already uses.

**`Advisory → Live Voice Evacuation Execution` / `Advisory → Live Building Control Execution`: remain ❌ NOT built — confirmed, investigated, and intentional**, exactly mirroring Milestone 5's own `AI → Decision Policy` boundary. `VoiceEvacuationPanel.show_live()`/`BuildingControlsPanel.show_live()` render `AdvisoryReport.civilian_announcements`/`.building_recommendations` directly as read-only "RECOMMENDED"/"Execution Provider: Not Connected" rows — neither method, nor any file this milestone touches, imports `voice_evacuation.controller`/`voice_evacuation.provider`/`speaker_manager`/`building_control.controller`/`building_control.providers` (mechanically enforced by `tests/test_live_command_center.py::CommandCenterLiveIntegrationGuardTests`, scoped to every file this milestone added or touched — `command_center/incident_data.py`, which legitimately uses those same modules for its own, already-audited Replay reconstruction, is deliberately excluded from that guard, not exempted from scrutiny).

**Still explicitly NOT connected (as of Milestone 6):**
- `Advisory → Live Voice Evacuation Execution` / `Advisory → Live Building Control Execution` — confirmed, investigated, remains not built (above). (Superseded below — Milestone 7 builds this connection, gated behind an explicit human operator action; see §19.)
- `AI Inference → Decision Policy` — unchanged from Milestone 5; still not built.
- Physical CCTV (`RTSPFrameSource`, a real `HumanDetector`, Live ReID) — unchanged, still correctly out of scope.
- A bounded live-history buffer for timeline scrubbing — Live mode's timeline controls are honestly disabled (`Dashboard.set_mode()`) rather than fed a fabricated scrub range; only a bounded *recent-events* list (Phase 15) was built, not full scrubbing.
- No building control execution, no automatic voice broadcast, no firefighter command authority, and no AI inference/advisory generation running inside Command Center itself were introduced — mechanically enforced.

## 19. Milestone 7 — Live Operator Action Routing (COMPLETED — `Command Center Operator → Voice Controller` / `Command Center Operator → Building Control Controller` now IMPLEMENTED; `Advisory/AI → Voice/Control Execution` confirmed to remain, structurally, NOT built)

Status: **an explicit, human-in-the-loop operator workflow now connects the live `AdvisoryReport` already reaching Command Center (Milestone 6) to the pre-existing Voice Evacuation and Building Control frameworks — but only ever through an operator's own Approve/Reject click, never automatically.** Full detail: `docs/architecture/live_operator_action_routing.md`.

**What was built:** `command_center/live_operator_action_gateway.py` (new) — `LiveOperatorActionGateway`, the one seam Command Center is allowed to route explicit operator intent through to `voice_evacuation.controller.VoiceEvacuationController`/`building_control.controller.BuildingControlController`. Lives in `command_center/`, not `live_system/`, specifically *because* `live_system` is already mechanically forbidden (Milestone 1's own `LiveSystemPackageDependencyDirectionTests`) from importing either controller — this milestone relies on that pre-existing guard rather than weakening it. `voice_evacuation.provider.VoiceOutputProvider` gained an `is_simulation_only` flag mirroring `building_control.providers.BuildingControlProvider`'s own established one; both controllers gained a trivial read-only `provider` property. `VoiceEvacuationPanel.show_live()`/`BuildingControlsPanel.show_live()` gained an optional `gateway` parameter and real Approve/Reject buttons — still importing nothing execution-capable directly (the pre-existing `CommandCenterLiveIntegrationGuardTests` guard needed zero changes). `Dashboard.set_operator_action_gateway()`/`MainWindow.enable_live_mode(..., operator_action_gateway=None)` thread an opaque gateway reference through, never inspecting it.

**`Command Center Operator → Voice Controller` / `Command Center Operator → Building Control Controller`: now ✅ IMPLEMENTED**, gated entirely behind an explicit operator action and a configured provider (`NO_PROVIDER`/`SIMULATION`/`LIVE_HARDWARE` capability, the last architectural-only). **`Advisory → Live Voice/Control Execution` (automatic): remains ❌ NOT built — confirmed, mechanically enforced, and now the deliberate final state**, not a gap: `tests/test_live_operator_action_routing.py::AIAuthorityGuardTests` proves `advisory_system/`, `live_system/live_ai_gateway.py`, `live_system/live_advisory_gateway.py`, `live_system/orchestrator.py`, and `decision_policy/` never import `voice_evacuation`, `speaker_manager`, `building_control.controller`, `building_control.providers`, or `command_center.live_operator_action_gateway` itself.

**Voice Controller → Physical PA:** still ❌ DISCONNECTED — only `SimulationVoiceOutputProvider` exists.
**Building Control Controller → Physical BMS/Fire Systems:** still ❌ DISCONNECTED — only `SimulationControlProvider` exists.

**A real gap found and fixed during this milestone:** re-ingesting the same `AdvisoryReport` after an operator's own Reject (which happens on every panel re-render, including the one the reject itself triggers) would have silently resubmitted the identical, just-rejected recommendation as a fresh `PENDING_APPROVAL` request — `RequestStatus.REJECTED` is terminal, so `BuildingControlController`'s own live-duplicate guard no longer protects it. `LiveOperatorActionGateway` now tracks rejected `source_recommendation_id` values and skips resubmitting exactly that recommendation, never blocking a genuinely different one.

**Still explicitly NOT connected:**
- `AI Inference → Decision Policy` — unchanged, still not built.
- Physical CCTV, PA hardware, BMS/fire-system protocols — unchanged, still correctly out of scope; no hardware communication of any kind was implemented.
- Authentication/operator identity — no user-management system exists; every operator action is attributed to the neutral, honest constant `COMMAND_CENTER_OPERATOR`, never a fabricated username.
