...
**UPDATE — CCTV Connection & Calibration Readiness milestone:** the real `FrameDecoderBackend` named above as missing now exists: `human_detection.opencv_decoder_backend.OpenCVFrameDecoderBackend`, real `cv2.VideoCapture`-backed, proven against real `vtest.avi` frames through the unmodified `RTSPFrameSource` and the real `YOLOHumanDetector`/`SimpleSingleCameraTracker`/`LiveOccupantManager`/`BuildingStateEstimator` chain — no `FakeRTSPBackend` anywhere in that proof. See `docs/architecture/cctv_connection_and_calibration_readiness.md` for the full milestone record and final per-component classification.
