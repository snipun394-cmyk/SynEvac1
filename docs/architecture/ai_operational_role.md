...
## Addendum E — Per-Candidate Predictive AI Data Foundation (follow-up milestone, completed)

§15's scoped-but-not-scheduled next milestone was carried out: `docs/architecture/localized_predictive_ai_dataset.md` (commit history following `fb123e6`). Summary, so this document does not go stale about its own "not this one":

- A simulation-side per-candidate extractor now exists (`predictive_dataset.simulation_extractor`), producing `Crowd-Intelligence`-shaped per-candidate features (queue length, approaching count, congestion level, capacity, adjacent-zone occupancy, traversability, walking distance) directly from an already-completed scenario run's `MultiAgentSimulationResult` — driving the simulator's own internal per-edge interval data, exactly option (1) of §15's three-part scope.
- A matching live-side extension exists (`predictive_dataset.live_extractor`), reusing `crowd_intelligence.engine.CrowdIntelligenceEngine`'s already-built per-candidate `AssetApproachMetrics` rather than building a second intelligence engine — option (2) of §15's scope, with fields correctly classified `LIVE_OBSERVABLE`/`LIVE_ESTIMABLE` per `ai_features.feature_schema`'s existing vocabulary (reused, not duplicated).
- Horizon-bounded, per-timestep relabeling of recorded scenario timelines now exists (`predictive_dataset.target_generator`) — an analysis pass over already-recorded `OccupantTimelineStep` data, not a simulator redesign — option (3) of §15's scope.
- **Simulation/live parity is genuinely established** (not merely asserted): structural fields and the shared congestion-classification code path match exactly under a controlled matched-state test; divergent-methodology fields (`candidate_queue_length`/`candidate_approaching_count`) are honestly disclosed as different mechanisms answering the same semantic question, never forced into fake equality; missing live evidence is reported as `None`, never fabricated.
- §10's proposed 20–30s horizon is now empirically corroborated, not merely proposed: 20s is the shortest horizon clearing both a genuine-advance-warning floor and a statistical-usability floor over a real 40-scenario, 41,940-row campaign.

**Still true, unchanged by this addendum**: no model has been trained on this new schema, `evacuation_recommendation.scoring.score_candidate()` is unchanged, and `ai_bottleneck_probability` remains the only AI term currently wired in (§1/§14, still rank-inert, still by design). The next actual step — training and validating a localized model against this new dataset, then wiring a new `ai_localized_bottleneck_risk` term per §13 — remains a distinct, not-yet-scheduled future milestone.
