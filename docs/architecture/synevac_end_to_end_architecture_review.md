...
1. **Wire `build_live_runtime()` into an actual runnable application entry point.** This is the single highest-priority gap: every subsystem audited above (Command Center, all five intelligence engines, Recommendation/Guidance/Signage, operator authority) is well-built and internally correct, but **unreachable from `main.py`**. A minimal fix would be a new launcher (not necessarily touching `main.py`/Designer at all) that constructs a `Building` (loaded from a `.syn` file), calls `build_live_runtime()`, starts the orchestrator, constructs `command_center.main_window.MainWindow`, and calls `enable_live_mode()` — essentially promoting `scripts/run_physical_camera_validation.py`'s own composition pattern into a first-class, GUI-attached entry point. Without this, "physical CCTV validation readiness" (the project's own stated near-term goal) has no application to actually validate against outside a script.
2. **Reconcile or explicitly document the `BuildingState.zone_occupancy` vs. `CrowdIntelligenceSnapshot.occupant_count` duplication** (§10, finding 1) — the one duplication risk found that could genuinely disagree at runtime today, unlike the others (which are intentional or dormant).
