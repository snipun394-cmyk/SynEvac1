...
|---|---|---|---|
| ~~`live_system/integration.py`'s Phase-7 gateways (`PerceptionGateway`, `AIInferenceGateway`, `DecisionPolicyGateway`, `CommandCenterGateway` + their sole adapters) are dead in production~~ **RESOLVED** | ~~CRITICAL~~ | See `docs/architecture/live_runtime_architecture_cleanup.md` | The four concrete adapters (`SensorFusionPerceptionGateway`, `PredictorAIInferenceGateway`, `GeneratePolicyDecisionPolicyGateway`, `DashboardCommandCenterGateway`) and their two adapter-only type aliases were deleted (zero production callers, confirmed by a full usage audit) by the Live Runtime Architecture Cleanup milestone. The four Protocols + `RecommendationBuilder` were kept, explicitly labeled legacy, because `LiveOrchestrator`'s own unmodified constructor signature still type-annotates against them. |
...
**This independently corroborated Phase 1's own finding from a different angle**: it wasn't only that `main.py` never called `build_live_runtime()` (since resolved — see `docs/architecture/application_live_runtime_integration.md`) — a whole PRIOR generation of gateway abstraction (`live_system/integration.py`) was already superseded by the current `Engine*Gateway` generation and left in place, unused, never deleted. Two dead layers stacked on top of each other. Both are now resolved: the entry-point gap by the Application Live Runtime Launcher milestone, the superseded gateway generation by the Live Runtime Architecture Cleanup milestone.
...
3. ~~**Delete or explicitly mark deprecated the two dead gateway generations**~~ **PARTIALLY RESOLVED** — `live_system/integration.py`'s Phase-7 gateways are deleted/legacy-labeled (Live Runtime Architecture Cleanup milestone, see `docs/architecture/live_runtime_architecture_cleanup.md`). Still open, deliberately untouched by that milestone (out of scope — no AI ranking/model changes): whether `LiveAIInferenceGateway`'s concrete classes (`RegistryLiveAIInferenceGateway`/`ThrottledLiveAIInferenceGateway`) should gain a real default-construction path or be left fully caller-supplied — this is a "correctly-built-but-unconfigured" question (no trained model to wire in yet), not a superseded-architecture one, and belongs with priority 4 below, not this cleanup.
4. ~~**Decide AI's actual role before investing further in it.**~~ **RESOLVED (architecture decision made, not yet implemented)** — see `docs/architecture/ai_operational_role.md` (Define AI's Real Operational Role milestone). §6's finding is confirmed correct and mechanically proven (not merely observed): `ai_bottleneck_probability` is a single whole-building value threaded identically into every exit candidate's score, structurally incapable of changing relative ranking, because the trained model's own feature schema (`CANONICAL_LIVE_SCHEMA`) has zero per-zone/per-asset fields — even retraining with `BottleneckModel`'s already-existing `target="location"` option on the current feature set would be scientifically indefensible. Decision: **Option D (hybrid)** — deterministic safety → measured live intelligence (already real, already rank-affecting via Crowd Intelligence/Evacuation Progress/Trajectory) → predictive localized AI (the one genuinely missing layer) → recommendation scoring → deterministic guidance. Recommended next step is narrowly scoped (a simulation-side extractor for per-candidate features, matching live-side schema extension, horizon-bounded relabeling of existing simulation timelines) and explicitly deferred to a future milestone — no model was trained, no scoring code was changed by this decision.
...
- **AI model retraining or new AI features** — priority 4's architecture decision is now made (`docs/architecture/ai_operational_role.md`), but the prerequisite simulation/feature-schema work it identifies (a per-candidate feature extractor, matching simulation-side data, horizon-bounded labels) has not been built yet; training a localized model before that exists would still be premature.
