...
**Correction (found by the follow-up `docs/architecture/predictive_dataset_campaign_v1.md` milestone's own Phase 4/5 checks, run at real campaign scale):** `candidate_approaching_count`'s simulation-side definition described below (§6/§7, "occupants whose fixed Route's final edge is this candidate") made the feature structurally, permanently zero for every Door and Stair candidate — a complete evacuation Route always ends at an Exit, so "final edge" can only ever be an Exit edge. A first fix (using the occupant's immediate next not-yet-started edge) turned out to be ALSO structurally zero for every candidate type, because this simulator's occupant movement model has no observable gap between finishing one edge and joining the next edge's queue. The definition now in `predictive_dataset/simulation_extractor.py` counts any not-yet-reached edge in the occupant's remaining route (still a plan-derived, causally-prior-to-`time` fact, no leakage implication changes) — see that milestone's own documentation (and `simulation_extractor.py`'s own "BUG HISTORY" comment) for the full story and the corrected campaign numbers. The rest of this document's design (candidate identity, sampling, target semantics, leakage boundary, E1/E2 differentiation, parity methodology) is unaffected and remains accurate.
