...
class MultiAgentSimulation:
    ...
    def add_occupant(
        ...
        # the one and only place a route is ever computed. Fixed for
        # the rest of this occupant's life in the simulation.
        ...
        start_node = occupant.route.nodes[0]
        self._add_node_occupant(start_node.id, occupant_id)

        heapq.heappush(
            self._event_heap,
            (depart_time, next(self._counter), self.TRY_ENTER_EDGE, occupant_id),
        )
    ...
    def _process_next_event(self):
        ...
        time, _, kind, occupant_id = heapq.heappop(self._event_heap)
    ...
    def _admit_onto_edge(self, time, occupant, edge):
        ...
        heapq.heappush(
            self._event_heap,
            (end_time, next(self._counter), self.ARRIVE_AT_NODE, occupant.occupant_id),
        )
    ...
    def _handle_arrive_at_node(self, time, occupant):
        ...
        if queue:
            ...
            heapq.heappush(
                self._event_heap,
                (time, next(self._counter), self.TRY_ENTER_EDGE, next_occupant_id),
            )
        ...
        heapq.heappush(
            self._event_heap,
            (time, next(self._counter), self.TRY_ENTER_EDGE, occupant.occupant_id),
        )
