...
from predictive_dataset.campaign_config_v3 import CampaignConfigV3
...
# =====================================================
# Predictive Dataset V3 milestone, Phase 12/13 -- shared, memory-safe
# campaign execution loop, used by BOTH the pilot (Phase 8) and
# full-scale (Phase 13) runners so the two campaigns are guaranteed to
# use identical simulation/extraction/labeling logic (only WHICH
# variants and HOW MANY scenarios per variant differ between callers).
#
# Streams rows directly to CSV inside the tick/candidate loop (never
# accumulates rows in memory) -- the exact discipline scripts/
# run_predictive_dataset_campaign_v2_2_fullscale.py and scripts/
# run_predictive_congestion_target_v2_relabel.py already established,
# reused here rather than reinvented (Phase 12's own "prefer existing
# chunked/streaming infrastructure where possible" instruction). A
# periodic psutil availability check (_check_memory) aborts loudly
# rather than risking the kind of near-OOM quality_checks.py's own
# duplicate-row-check incident already documented for V2.
#
# Target V2 ONLY -- Target V1 is never computed here (V1 is preserved,
# reproducible from its own untouched module, but is explicitly NOT to
# be used for new predictive training per this milestone's own
# instructions). Column names keep the V2-relabel script's
# "*_v2"-suffixed convention (target_v2, currently_congested_v2, ...)
# so scripts/train_localized_predictive_model_v3.py's existing loading/
# feature-matrix code can be pointed at a V3 CSV with minimal changes
# (Phase 18's exploratory generalization experiment).
# =====================================================
...
CSV_COLUMNS = (
    ...
    + BASE_FEATURE_NAMES + EXPERIMENTAL_FEATURE_NAMES + LABEL_COLUMNS
)
...
def run_campaign_v3(variants, config: CampaignConfigV3, output_dir: Path, *, log_every: int = 50) -> Dict[str, Any]:
    """Runs one campaign (pilot or full-scale) over `variants`
    (predictive_dataset.topologies_v3.StructuralVariant, with
    scenario_count already overridden via with_scenario_count by the
    caller). Writes candidate_dataset_v3.csv (streamed) and returns the
    scenario_metadata list plus a summary report dict -- callers write
    their own report.json (pilot vs full-scale need different analysis
    sections) but share this exact generation/extraction/labeling core."""

    output_dir.mkdir(parents=True, exist_ok=True)
    csv_path = output_dir / "candidate_dataset_v3.csv"
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as csv_file:
        ...
        for variant in variants:
            ...
            signature = compute_structural_signature(variant.family, variant.variant_id, spec.building)

            request = BatchGenerationRequest(
                definition=spec.definition, definition_id=f"predictive-dataset-v3-{variant.variant_id}",
            ...
            for scenario in iter_batch(request):
                ...
                try:
                    ...
                    scenario_metadata.append({
                        ...
                        "topology_family": variant.family,
                        "structural_variant_id": variant.variant_id,
                        "floor_count": signature.floor_count, "zone_count": signature.zone_count,
                        "exit_count": signature.exit_count, "stair_count": signature.stair_count,
                        "door_count": signature.door_count, "candidate_count": signature.candidate_count,
                        "mean_alternative_route_count": signature.mean_alternative_route_count,
